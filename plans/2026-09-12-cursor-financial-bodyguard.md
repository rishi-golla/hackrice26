...
- [ ] Implement speech provider adapters after checking current official APIs. `/speech/transcribe` accepts only allowed audio MIME types, <=10 MB and <=30 seconds; audio is memory-only and deleted after completion/error. `/speech/synthesize` accepts a session-owned generated reply ID; arbitrary renderer text cannot become a financial answer. Bound network operations with deadlines; failures expose retry/typed input rather than hanging.
