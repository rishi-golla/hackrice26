...
struct RealtimeVoiceCheck {
    ...
    static func main() async throws {
        ...
        let audio = BuddyPCM16AudioConverter(targetSampleRate: 24000).convertToPCM16Data(from: buffer)!
        ...
        client.start(recordedAudio: audio) {
        ...
        print("PASS: conversation history, screen image, real audio input, research tool round trip, streamed speech playback, transcript, and completion")
        client.start(recordedAudio: audio) { FlickyRealtimeContext(instructions: "Reply briefly.", history: [], images: []) }
        client.cancel()
