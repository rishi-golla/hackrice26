...
- **Analytics**: PostHog via `PeppaPriceAnalytics.swift`
...
**Transient Cursor Mode**: When "Show PeppaPrice" is off, pressing the hotkey fades in the cursor overlay for the duration of the interaction (recording → response → TTS → optional pointing), then fades it out automatically after 1 second of inactivity.
...
| `PeppaPriceAnalytics.swift` | ~121 | PostHog analytics integration for usage tracking. |
...
`Subscriptions.swift` stores user-added real subscriptions and unclassified Nessie recurring-bill candidates separately. Recurring rent/utilities do not automatically count as subscriptions. Dates are recorded, never extrapolated; real details can be edited. Counts describe the selected app account's tracked list, not all real-world subscriptions. `[SUBSCRIPTIONS]` opens review from voice/text; Options → Subscriptions opens it directly. Ask which renewals to keep or cancel.

`SubscriptionPanel.swift` provides Keep/Cancel review and an embedded provider browser. `SubscriptionCancellation.swift` uses an account-isolated persistent WebKit profile and bounded Claude-selected DOM controls after explicit account/cancellation confirmation. The model cannot initiate cancellation through a response tag. It never enters credentials, accepts fees/agreements, buys, deletes accounts, or changes plans. Authentication, CAPTCHA, unsupported UI and domain changes pause for the user. Only an exact observed provider cancellation statement yields a saved receipt; a request, open URL or email draft is not a completed cancellation. Interrupted jobs require review, never automatic replay. Stop and account changes cancel pending work.

Real subscription discovery and outbound email are not connected. The provider's observed cancellation/support mailto link can prepare an email draft for the user; no legal claims or invented recipients, and no automatic sending. Nessie sandbox candidates cannot trigger real provider cancellation. Subscription files are owner-only and account-scoped; form values, URL tokens, raw control identifiers, and browser cookies never enter the planner prompt. Validate with `bash scripts/checks/run-subscription-checks.sh`; browser checks use injected local HTML and never alter real subscriptions. Full app builds use Xcode.

## Local gateway configuration

The installed gateway URL is read from `~/Library/Application Support/Flicky/runtime.plist` under `FLICKY_WORKER_URL`, with bundled Info.plist as fallback. Keep local configuration owner-only and out of source control. Worker package names are product-branded; a configuration rename does not deploy or migrate a live service. Required inherited license terms are centralized at `../THIRD_PARTY_NOTICES.txt`.

Use PeppaPrice product naming in all new source, assets, docs, and plans. Keep inherited author attribution only in the root third-party license notice; do not restore obsolete upstream branding.

## Credit demo UI update

Borrowing requests such as “I need $8,000 for 36 months” or “I need five thousand” now open the bank comparison directly, bypassing dummy entry. `CreditBorrowingRequest` parses amount, term, and requested APR; transcript, typed entry, and Realtime research routes present the panel immediately. Preserve official-site navigation and ordinary shopping/account queries. Requested terms are transient, reset with account/session changes, and do not personalize published bank examples. When all three values are supplied, the existing payment engine renders a clearly hypothetical no-fee monthly payment. Voice introduces a couple of options and their rates, fees, and tradeoffs; never claim an offer or credit pull. This supersedes explicit-simulation-only routing elsewhere in this file.
...
Every recommendation should explain one or two relevant selected-customer facts, source/time window, and how they change the suggestion. Shared runtime rules live in FlickyPersonaConfig.content, used by voice and Claude. The credit research shortcut must fetch current account evidence before returning guidance; never bypass personalization with canned speech. FinancialModels includes source coverage and a full observation date. Single-period aggregates support spending patterns, not increasing/decreasing trends. Missing evidence makes advice conditional; sandbox data and example credit scores never establish real affordability, eligibility, or personalized rates. Definitions and navigation do not need unrelated account facts.
