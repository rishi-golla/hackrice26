...
export function createElevenLabsTranscriber(options: SpeechFetchOptions) {
  return {
    async transcribe(audio: Uint8Array, mime: string, durationMs: number) {
      ...
      form.append('file', new Blob([audio], { type: mime }), 'voice-input');
...
export function createElevenLabsSynthesizer(options: SpeechFetchOptions & { voiceId?: string; modelId?: string }) { return { async synthesizeValidatedReply(text: string) { if (!options.apiKey || !options.voiceId) throw new ProviderUnavailableError('ElevenLabs unavailable: credentials missing'); if (!text.trim() || text.length > 4000) throw new Error('Invalid reply text'); const response = await fetchWithDeadline(options.fetchImpl ?? fetch, `https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(options.voiceId)}?output_format=mp3_44100_128`, { method: 'POST', headers: { 'xi-api-key': options.apiKey, 'content-type': 'application/json' }, body: JSON.stringify({ text, ...(options.modelId ? { model_id: options.modelId } : {}) }) }, options.timeoutMs ?? 15_000, 'synthesis'); if (!response.ok) throw new Error(`ElevenLabs synthesis failed with HTTP ${response.status}`); return new Uint8Array(await response.arrayBuffer()); } }; }
