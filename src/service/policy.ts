...
export function requireVerification(session: CappySession, accountId: string, scope: VerificationScope, gate: VerificationGate = unavailableVerificationGate) {
  const scopedAccountId = assertAccountAccess(session, accountId);
  if (!gate.isConfigured()) throw new VerificationError('verification_unavailable', 'Identity verification is unavailable.');
  if (!gate.isApproved(session, scopedAccountId, scope)) throw new VerificationError('verification_required', 'Identity verification is required.');
